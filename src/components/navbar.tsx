  ...
  <div className="flex-1 flex items-center justify-between md:items-stretch md:justify-between md:mx-2">
    ...
    <div className="hidden md:inline-flex md:space-x-6">
      {links.map((link, index) => (
        <Link to={link.location} key={`location-${index}`}>
          ...
          >
            <span className={location === link.title ? "underline" : ""}>
              {link.title}
            </span>
          </button>
        </Link>
    ...
    <div className="hidden md:block md:ml-6">
      <div className="flex space-x-2 mr-2">
        <span
          key="nav-dark-mode"
          className="px-2 pt-1 pb-0.5 rounded-xl h-auto"
        >
          <DarkModeSwitch />
        </span>
      </div>
...
const Mobile = ({ location }) => (
  <Disclosure.Panel className="md:hidden">
    <div className="flex flex-col px-0 pb-3 mr-4 space-y-1 md:block md:px-2">
      <span
        key="nav-dark-mode"
        className="rounded-xl text-lg font-medium h-auto mb-4"
      >
        <DarkModeSwitch />
      </span>
      {links.map((link, index) => (
        <span
          className="rounded text-lg font-medium h-auto mb-4"
...
export default function Navbar({ siteTitle, location }) {
  return (
    <Disclosure
      as="nav"
      className="bg-blue-300 dark:bg-slate-700 text-white space-x-4"
    >
      {({ open }) => {
        return (
          <>
