...
const Hamburger = ({ open }) => (
  <div className="absolute inset-y-0 right-4 flex items-start md:hidden">
    <Disclosure.Button
      ...
      text-gray-100 hover:text-white hover:bg-slate-400 focus:outline-none focus:ring focus:ring-inset focus:ring-white"
    >
      ...
      {links.map((link, index) => (
        <Link to={link.location} key={`location-${index}`}>
          <button
            ...
            font-medium tracking-wider uppercase px-1.5 py-2 rounded duration-200"
          >
...
const Mobile = ({ location }) => (
  <Disclosure.Panel className="md:hidden">
    <div className="flex flex-col mr-4 space-y-1 md:block py-3">
      {links.map((link, index) => (
        <span className="rounded text-lg font-medium h-auto" key={`mobile-nav-${index}`}>
          <Link to={link.location} key={`location-${index}`}>
            <button
              type="button"
